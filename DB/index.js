...
const uri = `mongodb+srv://${DB_USER}:${DB_PASS}@${DB_HOST}/moviesapi`
...
const listDatabases = async (client) => {
  ...
  const createListing = async (client, newListing) => {
    const result = await client.db('moviesapi').collection('movies').insertOne(newListing)
  ...
  async function findMovieByTitle(client, title) {
    ...
    console.log(re)
    const result = await client.db('moviesapi').collection('movies').findOne({ title: re })
  ...
  async function findMovieByID(client, ID) {
    const result = await client.db('moviesapi').collection('movies').findOne({ _id: ObjectId(ID) })
  ...
  async function findMovieManyByTitle(client, title) {
    re = new RegExp(title, 'i')
    console.log(re)
    const cursor = await client.db('moviesapi').collection('movies').find({ title: re })
  ...
  async function findMovies(client) {
    let res = []
    const cursor = await client.db('moviesapi').collection('movies').find()
  ...
  async function deleteMovieByID(client, id) {
    const result = await client.db('moviesapi')
    .collection('movies')
    .deleteOne({ _id: id})
